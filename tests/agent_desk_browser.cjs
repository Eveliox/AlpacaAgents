...
(async () => {
  ...
  try {
    ...
    assert(await evaluate("document.getElementById('desk-panel').open"),'workflow is prominent on first load');
    assert.equal(await evaluate("document.querySelectorAll('.desk-task').length"),6);
    assert.match(await evaluate("document.getElementById('desk-outcome').textContent"),/Attention/);
    assert.equal(await evaluate("document.getElementById('desk-progress-meter').value"),0);
    assert(await evaluate("!document.querySelector('#desk-inspector details').open"),'technical evidence is progressive disclosure');
    ...
    assert.equal(await evaluate("document.getElementById('desk-progress-meter').value"),5);
    assert.equal(await evaluate("getComputedStyle(document.querySelector('.desk-connectors path[data-link=prior]')).animationName"),'none','unbuilt models must not look active');
    ...
    await evaluate("document.getElementById('desk-motion').click()");
    assert.equal(await evaluate("document.getElementById('desk-motion').getAttribute('aria-pressed')"),'true');
    assert.equal(await evaluate("document.querySelectorAll('.desk-agent.lit').length"),6,'pause settles to readable evidence');
    assert.match(await evaluate("document.getElementById('desk-replay-status').textContent"),/Motion paused/);
    await pause(900);
    assert(await evaluate("!document.querySelector('.desk-swarm').classList.contains('flowing')"),'cancel pending replay callbacks');
    await evaluate("document.getElementById('desk-replay').click()");
    assert.equal(await evaluate("getComputedStyle(document.querySelector('.desk-connectors path.lit')).animationName"),'none','replay respects pause');
    await evaluate("document.getElementById('desk-motion').click()");
    assert.equal(await evaluate("document.getElementById('desk-motion').getAttribute('aria-pressed')"),'false');
    ...
    for(let n=0;n<20&&!(await evaluate("document.getElementById('desk-motion').disabled"));n++)await pause(50);
    assert(await evaluate("document.getElementById('desk-motion').disabled"),'system reduced motion cannot be overridden');
    await evaluate("document.getElementById('desk-replay').click()");
